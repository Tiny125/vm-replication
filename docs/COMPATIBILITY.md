...
- **Boots from the Linode's local disk** (NVMe — faster, and the boot disk costs nothing beyond the plan). Local-disk boot is **multi-disk**: the first disk becomes the instance's own local disk and every further disk is cloned into a Block Storage volume attached to the same instance (up to 7 of them — `sda` holds the boot disk, leaving `sdb`–`sdh`). Pick the launch plan from a Shared/Dedicated list — only plans whose disk fits the **boot** disk are offered — and the form shows the estimated monthly Block Storage cost for any data disks alongside the plan price.
