...
| Message | Meaning | Remediation |
|---|---|---|
| `the source went offline mid-pass during cutover — expected if you just powered it off` (info) | You powered the source off (step 2) while its agent still had a connection open; the receiver noticed the peer vanish. Since cutover step 1 now **drains** — it waits for any in-flight pass to finish before freezing — the frozen image was already complete and consistent when this happened. | **None.** Proceed with the cutover. (A *red* `replication attempt failed: stream closed before done` outside of cutover is different — that means the source died or lost network mid-replication and the agent will retry.) |
| `cutover: a replication pass was still running after 3m0s and was severed` (warn) | A pass was still streaming when the freeze's drain grace expired (rare — deltas normally finish in seconds; this suggests a very busy source or a stalled connection). The image may mix two points in time; the convert's forced `fsck` usually repairs it. | Prefer a clean image: click **Cancel** (replication resumes), let one `delta sync complete` land in the activity log, then cut over again during a quieter moment. Or proceed — the no-root guard aborts before launching anything unusable. |
