...
It downloads the prebuilt release tarball matching this machine's CPU architecture (linux/amd64 or linux/arm64), <b>verifies its SHA-256 against the release's <code>SHA256SUMS</code> before extracting anything</b>, unpacks it to <code>/usr/local/src/vm-replication-&lt;version&gt;</code> (with a stable <code>/usr/local/src/vm-replication</code> symlink to it), and runs the installer from there. No Go toolchain, no compiler, no <code>git clone</code> needed — the binaries are prebuilt and static. To pin an exact release instead of always installing latest: <code>sudo VMREPL_REF=v1.4.0 bash bootstrap.sh</code>.</li>
