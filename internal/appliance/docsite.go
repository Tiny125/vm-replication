...
It downloads the prebuilt release tarball matching this machine's CPU architecture (linux/amd64 or linux/arm64), <b>verifies its SHA-256 against the release's <code>SHA256SUMS</code> before extracting anything</b>, unpacks it to <code>/usr/local/src/vm-replication-&lt;version&gt;</code> (with a stable <code>/usr/local/src/vm-replication</code> symlink to it), and runs the installer from there. No Go toolchain, no compiler, no <code>git clone</code> needed — the binaries are prebuilt and static. To pin an exact release instead of always installing latest — recommended for production, so a later re-run cannot silently move you to whatever is newest — pass the tag through the pipe (substituting the release you want):
<div class="codeblock"><pre>curl -fsSL https://raw.githubusercontent.com/Tiny125/vm-replication/main/scripts/bootstrap.sh | sudo VMREPL_REF=v0.1.1 bash</pre><button class="copy" onclick="cp(this)">Copy</button></div>
Re-running is safe and upgrades in place: an existing region and port are preserved.</li>
...
<h2>Disk boot</h2>
<p>Every migration is a <b>block-for-block</b> copy of every source disk — the exact disk contents, not just files — replicated continuously (an initial full sync, then deltas every ~60&nbsp;s). At cutover the boot image is converted for Linode and its <b>GRUB configuration is checked for errors before you're asked to power off the source</b> — that checks the configuration file, not a live boot; the guest's actual boot is verified separately, once you launch. The boot disk lands on the destination's own <b>local NVMe disk</b> at no extra cost, and any further disks become Block Storage volumes attached to the same instance.</p>
...
<li><a href="#cutover">Cut over</a>: stop the source, click <span class="btn-demo primary">Cutover instance</span> — the appliance takes a final pass and <b>checks the converted boot image's GRUB configuration for errors while the source is still running</b> (not a live boot — the guest's actual boot is verified separately after Launch).</li>
...
<ol class="steps">
<li><b>Stop replication &amp; prepare (this button).</b> The appliance takes one final consistent pass (the source root is briefly remounted read-only), then <b>converts the boot image and checks its GRUB configuration for errors — before you power anything off</b>. This does not test a live boot; if the check fails, the cutover aborts with the reason and the source keeps running. The guest's actual boot is verified separately once you launch. If the source is already powered off or idle, tick <i>"skip the read-only snapshot"</i> in the dialog.</li>
