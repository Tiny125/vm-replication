...
| `read hello: unexpected EOF` / `read hello: EOF` | A client opened a TCP connection to the receiver port and **closed it immediately** without completing the handshake (classic TCP health check / scanner). | Same as above — harmless. |

> **Key point:** these are connections from **something other than your agent**.
> Current builds classify all of them (including the hello-stage `EOF`) as
> connection noise and keep them **out of the activity log** entirely — they only
> appear in the system log. If, in between them, you still see
> `agent connected, replicating` and the **bytes received** / **percentage**
> keep climbing, replication is healthy — the noise is just rejected probes.
> They become a real concern only if your agent **never** connects and you see
> *only* these.
...
| Message | Meaning | Remediation |
|---|---|---|
| `the source went offline mid-pass during cutover — expected if you just powered it off` (info) | You powered the source off (step 2) while its agent still had a pass in flight; the receiver noticed the peer vanish. Harmless: **delta passes are applied atomically** — blocks are staged and only written to the volume once the whole pass has arrived — so the interrupted pass was **discarded whole** and the frozen image is exactly the last complete pass. | **None.** Proceed with the cutover. |
| `cutover: a replication pass was still running after 3m0s and was severed; its staged data was discarded whole` (warn) | A pass was still streaming when the freeze's drain grace expired, so the receiver cut it. Because delta passes apply atomically, the severed pass never touched the volume — the frozen image is the last completely applied pass, just slightly older than it could have been. | Usually none. If you want the very freshest data in the image, click **Cancel**, let one `delta sync complete` land in the activity log, and cut over again. |
| First boot of a migrated RHEL-family instance is slow, console shows relabeling | The source had **SELinux enforcing**, so the convert scheduled a one-time full **relabel** (`/.autorelabel`) — required because config files written during conversion would otherwise be unlabeled and could break sshd/networking. | Wait it out (one boot); subsequent boots are normal. |
| Migrated instance: cloud-init / waagent / Google or AWS agents are disabled | The convert **disables the source cloud's agents** — their platform APIs don't exist on Linode (at best log noise, at worst multi-minute boot delays or account churn). Networking/hostname/root access are configured statically instead. | Re-enable anything you genuinely need (`systemctl enable <unit>`; remove `/etc/cloud/cloud-init.disabled` for cloud-init). |
| `WARNING: fstab swap entry '…' points at a device that was not migrated — disabled it` (convert log) | The source kept its swap on a **separate disk** (clouds and Linode attach one next to the boot disk) or a device that didn't come along. Only the boot disk is migrated, so the fstab entry would make every boot stall ~90s waiting for a device that never appears — the convert comments it out (marker `# vmrepl: disabled`), and the instance runs without swap. | Usually none. To add swap on the migrated instance, create a **swapfile**: `fallocate -l 512M /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile`, then add `/swapfile none swap sw 0 0` to `/etc/fstab`. |
| **I can't SSH into the file-transfer destination while the copy is running** (the root password set at Create destination stops working) | **Expected after the first complete pass.** The destination has taken on the source's `/etc/passwd`, `/etc/shadow`, `/etc/ssh` and `/root/.ssh`, because the whole point of the method is that it becomes the source. | Log in with the **source's** credentials, or use the Lish console. The activity log says this once, on the "file copy complete" event. |
| **The destination's receiver stops answering after a copy pass**, and the console still shows the destination ready | Fixed in current builds. The delete pass removed files the source did not send — and the receiver's own binary, certs and unit exist only on the destination, so they were never "seen". Unlinking a running binary does not kill it, so the pass completed and nothing looked wrong; but the unit is `Restart=always`, so the next restart failed 203/EXEC. Older builds also deleted `/dev/null` and `/run/sshd` on the live destination (both are unlinkable as root), which breaks `sshd` and `systemctl` mid-copy. | Upgrade the replication server so new destinations get the fixed receiver, then create the destination again. A destination launched before the upgrade still runs the old receiver. |
| **the appliance is low on disk: N free of M in /var/lib/vm-repl** (warn) | Replication **stages each delta pass** to a file in the data directory before applying it atomically, so an interrupted pass is discarded whole. That staging needs room for the pass's changed bytes, per disk, concurrently — so a busy multi-disk source can fill a small boot disk even though the replica itself lives on Block Storage volumes. | Free space on the appliance, or **cut over and Close** finished migrations (Close also removes their replication volumes). Resize the appliance's boot disk in Cloud Manager if it recurs. The warning appears once and clears when free space recovers. |
| **the appliance service restarted** (info) | `applianced` restarted — a deploy, an OOM, or a crash. Receivers are restarted automatically and replication continues. A copy pass that was in flight is discarded **whole**, never applied half-way, and the agent retries within ~60s. | None; it is informational. If it repeats, check `journalctl -u applianced` for the cause. |
| **The migrated instance stalls ~90s at every boot** waiting for a swap device, and the convert log shows **no** `fstab swap entry … disabled it` warning | **Stale swap entry that was wrongly judged live** — fixed in current builds. The convert decides whether an fstab swap device came along by resolving its UUID/LABEL, and it used to accept *any* match on the converting host. **Every Linode swap image ships the same UUID** (`f1408ea6-59a0-11ed-bc9d-525400000001`) and the same `linode-swap` label, so on a Linode replication server the source's stale entry resolved to the **appliance's own** swap disk and was left enabled. Current builds require the resolved device to be part of the **migrated disk** itself. | Upgrade the replication server and re-run the cutover, or fix the launched instance in place: comment out the swap line in `/etc/fstab` (`# vmrepl: disabled`) and reboot. Add a **swapfile** if you want swap — see the row above. |
| `WARNING: the image has no /root directory (heavy fsck repair?) — recreating it` (convert log) | The frozen image needed a heavy `fsck` repair (typically after a pass was interrupted mid-stream on an older build without the drain-then-freeze fix), and the repair dropped `/root`. The convert now recreates it (0700) and continues instead of crashing at `.convert-inner.sh: No such file or directory`. | The conversion proceeds, but a repair that lost `/root` may have relocated other entries: after the instance boots, check `/lost+found` and validate your application. For a pristine image, resync from the intact source and cut over again on a current build (step 1 drains, so this can't recur). |
...
| `cutover: the source could not deliver a read-only/point-in-time final pass … continuing the cutover from the current crash-consistent replicated data` (warn) | The read-only remount of the source root was refused because processes are still writing to `/` — **normal on any running system** (systemd/journald always hold the root open). The cutover **continues automatically** from the current crash-consistent data: it is `fsck`-repaired during conversion, and the boot image is **validated before you power off**, so nothing is lost. Older builds hard-failed here and asked you to tick "Skip the read-only snapshot". | Usually none — let step 1 finish and follow the card. For a byte-perfect final image, stop the source's apps (or power the source off) and cut over again. |
| The file-transfer destination never leaves *"installing the file receiver"*, even after pasting the manual install command (which printed `receiver started on :5999`) | On older builds the destination's systemd unit launched the receiver with a `-mode file` flag the binary didn't define, so it **exited instantly on every start** (`flag provided but not defined`) and systemd's restart loop hit its start limit — the port never actually listened, even though the install script printed success. Separately, the appliance stopped watching the port after 15 minutes, so even a working manual install done later was never noticed. | **Update the appliance** (both are fixed: the receiver accepts `-mode`, and the console keeps watching indefinitely), then click the card's retry / re-run the manual install command in the destination's Lish console. Confirm on the destination with `systemctl status vmrepl-receiver` — it must show `active (running)`, not `failed`. Also ensure nothing blocks the appliance and source from reaching the destination's public IP on **TCP 5999** (e.g. a Linode Cloud Firewall). |
...
| `the rescue copy did not power the instance off within <time>` | Disk-boot cutover boots the destination into **Rescue Mode** and waits for you to paste the one-line **copy command** (shown on the migration card) into its Lish console; the command streams the image from the appliance onto the local disk and powers the instance off when done. This error means that power-off never arrived — usually the command was **not run**, or the copy was interrupted (closed Lish tab kills nothing — the copy keeps running — but a dropped download makes `dd` exit early without powering off). The wait budget scales with the image size and also absorbs the time before you paste. | Click **Retry cutover** — it removes the stale instance, re-enters Rescue Mode, and shows a **fresh command** (the old token is invalidated). Paste it and watch the `dd` progress in the Lish session; when it prints "copy complete" the instance powers off and the appliance finishes automatically. If a copy was still running when the budget expired, just Retry — the redo is quick (no re-replication). If the instance reboots instead of staying off, disable the **shutdown watchdog (Lassie)** in its Settings and retry. |
...
| **Audit log bucket shows as "not created / no longer exists" but it still exists in Cloud Manager** | Older builds could wrongly mark the bucket gone. Two causes, both fixed: (1) when the appliance reused an already-existing bucket it stored only `{label, region}` and dropped the **storage cluster** — in regions where the cluster differs from the region (e.g. Singapore: region `sg-sin-2`, cluster `sg-sin-1`), log uploads then targeted the wrong endpoint and got a 404; (2) the uploader treated a single 404 as proof the bucket was deleted and cleared the card. Current builds store the bucket's real endpoint and **confirm with the account (ListBuckets) before clearing**, so a transient 404 can't cause a false negative. | Click **Refresh** on the token card — it re-checks your account, finds the bucket, and restores the status (re-storing the correct endpoint) without recreating anything. Update the appliance to stop it recurring. Your logs in the bucket are untouched throughout. |
